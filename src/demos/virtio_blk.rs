...
use crate::drivers::{self, virtio_blk, BlockDriver};
...
pub fn virtio_blk_demo() {
    ...
    let mut driver = match virtio_blk::VirtioBlk::new() {
        Ok(d) => d,
        Err(e) => {
    ...
    match driver.start_read(0, &mut buf.0) {
        Err(e) => { println!("✗ Failed to start read: {}", e); return; }
        Ok(()) => {}
    }
    ...
    let signature = ((buf.0[511] as u16) << 8) | (buf.0[510] as u16);
