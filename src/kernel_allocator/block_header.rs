...
pub(super) const BLOCK_HEADER_SIZE: usize = size_of::<BlockHeader>();
...
impl BlockHeader {
    pub(super) fn new(size: usize, used: bool) -> Self {
        BlockHeader {
            _size: size as u32,
            _prev: NULL_OFFSET,
    ...
    #[inline]
    pub(super) fn alloc_area_start(&self) -> *mut u8 {
        unsafe { (self as *const _ as *mut u8).add(BLOCK_HEADER_SIZE) }
    }

    #[inline]
    pub(super) fn next(&self) -> *mut BlockHeader {
        let this = self as *const BlockHeader;
        (this as usize + self._size as usize + BLOCK_HEADER_SIZE) as *mut BlockHeader
    }

    #[inline]
    pub(super) fn prev(&self) -> *mut BlockHeader {
        from_offset(self._prev)
    }

    #[inline]
    pub(super) fn set_prev(&mut self, prev: *mut BlockHeader) {
        self._prev = to_offset(prev);
    }

    #[inline]
    pub(super) fn free_next(&self) -> *mut BlockHeader {
        from_offset(self._free_next)
    }

    #[inline]
    pub(super) fn set_free_next(&mut self, next: *mut BlockHeader) {
        self._free_next = to_offset(next);
    }

    #[inline]
    pub(super) fn free_prev(&self) -> *mut BlockHeader {
    ...
    #[inline]
    pub(super) fn set_free_prev(&mut self, prev: *mut BlockHeader) {
    ...
    #[inline]
    pub(super) fn add_size(&mut self, size: usize) {
        self._size += size as u32;
    }
    ...
    #[inline]
    pub(super) fn set_used(&mut self) {
        self._free_prev |= 1;
    }
    ...
    #[inline]
    pub(super) fn end_ptr(&self) -> usize {
